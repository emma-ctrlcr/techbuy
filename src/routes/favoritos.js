...
// DELETE /api/favoritos/:id    — Quitar favorito (id_producto)
...
router.post('/', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // INSERT OR IGNORE (UNIQUE constraint evita duplicados)
    const { rows } = await pool.query(
    ...
    console.error('Error en POST /favoritos:', err);
    return res.status(500).json({ error: 'Error interno del servidor' });
  }
});

/* ── DELETE /api/favoritos/:id_producto ───────────────────── */
router.delete('/:id_producto', requireAuth, async (req, res) => {
  ...
  try {
    ...
    console.error('Error en DELETE /favoritos:', err);
    return res.status(500).json({ error: 'Error interno del servidor' });
  }
});
