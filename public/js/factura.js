...
function renderFactura() {
  ...
  document.getElementById('facClientDept').textContent  =
    (city ? city + ', ' : '') + dept;
  document.getElementById('facDate').textContent = capitalizeFirst(fecha) + ' · ' + hora;
  ...
  tbody.innerHTML = items.map(item => {
    ...
    return `
      <tr>
        ...
        <td class="fac-td-center fac-price">C$${unitSinIva.toFixed(2)}</td>
        <td class="fac-td-center fac-price" style="color:#6b7280">C$${(ivaUnit * item.qty).toFixed(2)}</td>
        <td class="fac-subtotal-cell">C$${lineTotal.toFixed(2)}</td>
      </tr>`;
  }).join('');

  // 7. Renderizar totales
  document.getElementById('facSubtotal').textContent = 'C$' + subtotalSinIva.toFixed(2);
  document.getElementById('facIva').textContent      = 'C$' + iva.toFixed(2);
  document.getElementById('facShipping').textContent = shipping === 0 ? 'Gratis' : (order.shipping != null ? 'C$' + shipping : 'C$' + shipping.toFixed(2));
  document.getElementById('facTotal').textContent    = 'C$' + total.toFixed(2);

  if (discount > 0) {
    document.getElementById('facDiscountRow').style.display = 'flex';
    document.getElementById('facDiscount').textContent = '-C$' + discount.toFixed(2);
  }
