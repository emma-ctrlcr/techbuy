...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  } catch(e) { console.warn('checkout.js:', e.message); }
...
function renderSummary() {
  ...
  itemsEl.innerHTML = TB_cart.map(i => `
    <div class="chk-sum-item">
      ...
      <div class="chk-sum-item-price">C$${((i.price * 1.15) * i.qty).toFixed(2)}</div>
    </div>`).join('');
  ...
  document.getElementById('chkSumSubtotal').textContent = 'C$' + withIva.toFixed(2);
  document.getElementById('chkSumIva').textContent      = 'C$' + iva.toFixed(2);
  document.getElementById('chkSumTotal').textContent    = 'C$' + final.toFixed(2);

  const shipRow = document.getElementById('chkShippingRow');
  const shipVal = document.getElementById('chkShippingVal');
  if (shipRow && shipVal) {
    if (shipping > 0) { shipRow.style.display='flex'; shipVal.textContent='C$'+shipping; }
    else              { shipRow.style.display='none'; }
  }
  const discRow = document.getElementById('chkDiscountRow');
  if (couponDiscount > 0) {
    discRow.style.display='flex';
    document.getElementById('chkDiscountVal').textContent='-C$'+discount.toFixed(2);
  } else {
...
async function handlePay() {
  ...
  if (TB_api.isLoggedIn()) {
    try {
      const items = TB_cart.map(i => ({ id_producto: i.id, cantidad: i.qty }));
      const body  = { tel, address, city, dept: dept||city, zip, items };
      if (couponCode) body.cupon = couponCode;

      const result = await TB_api.post('/pedidos', body);
      const pedido = result.pedido;

      // Guardar datos de factura (usar items del carrito para precios correctos)
      ...
      TB_cart=[]; TB_saveCart(); TB_updateCartUI();
...
function _finalizarPedidoLocal(tel, address, city, dept, zip) {
  const orderId = Date.now().toString().slice(-6);
  const subtotalBase = TB_cartTotal();
  const iva          = subtotalBase * 0.15;
  const withIva      = subtotalBase + iva;
  const discountAmt  = withIva * couponDiscount;
  const shippingAmt  = (typeof TB_shippingCost !== 'undefined') ? TB_shippingCost : 0;
  ...
  TB_cart=[]; TB_saveCart(); TB_updateCartUI();
  const el = document.getElementById('chkSuccessOrderId');
  if (el) el.textContent='Pedido #'+orderId+' registrado correctamente';
  showProcessingState('success');
  setTimeout(()=>{ window.location.href='factura.html'; }, 1800);
}
