...
async function renderFavorites() {
  ...
  favs.forEach(p => {
    p.id = p.id_producto || p.id;
    p.price = parseFloat(p.price ?? 0);
    p.stock = parseInt(p.stock) ?? 0;
    if (typeof TB_cacheProduct === 'function') TB_cacheProduct(p);
  });

  grid.innerHTML = favs.map(p => {
    const price = parseFloat(p.price ?? 0);
    const imgHTML = (p.imagenes && p.imagenes.length > 0)
      ? `<img src="${p.imagenes[0]}" alt="${p.name || p.nombre}" width="260" height="200" style="width:100%;height:100%;object-fit:contain;" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><i class="fa-solid ${TB_iconForCat(p.cat || 'otros')}" style="display:none;position:absolute;"></i>`
    ...
    <div class="prod-card" data-id="${p.id}">
      <div class="prod-img" style="position:relative;display:flex;align-items:center;justify-content:center;">
        <button class="perfil-fav-heart" onclick="event.stopPropagation();removeFavorite(${p.id})">
          <i class="fa-solid fa-heart"></i>
        </button>
        ${imgHTML}
      </div>
      <div class="prod-info">
        <div class="prod-brand">${p.brand || p.marca || ''}</div>
        <div class="prod-name">${p.name || p.nombre}</div>
        <div class="prod-price-row">
          <span class="prod-price">${TB_formatPrice(price)}</span>
        </div>
  ...
  grid.removeEventListener("click", TB_cardGridClickHandler);
...
async function renderOrders() {
  ...
  container.innerHTML = orders.map(o => {
    console.log('[Perfil] Pedido #' + (o.id_pedido || o.id) + ' fecha:', o.fecha);
    const dateStr = o.fecha ? TB_formatDate(o.fecha) : (o.date || '—');
    const totalDisplay = TB_formatPrice(
      o.total
        ? parseFloat(o.total)
        : (o.items || []).reduce((s, i) => s + parseFloat(i.precio_unitario || i.price || i.precio || 0) * (i.cantidad || i.qty || 1), 0)
    );
    ...
    <div class="perfil-order-card perfil-order-clickable"
      ...
      <div class="perfil-order-items">${itemsLabel || 'Ver detalle'}</div>
        <div class="perfil-order-total">Total: ${totalDisplay}</div>
...
async function handleEditSubmit(e) {
  ...
  else if (/\d/.test(nombre))           { setFieldError('editNombre',   'Sin números'); valid = false; }
  if (!apellido)                        { setFieldError('editApellido', 'Requerido'); valid = false; }
  else if (/\d/.test(apellido))         { setFieldError('editApellido', 'Sin números'); valid = false; }
