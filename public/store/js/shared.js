...
function TB_setupSharedHeader() {
  ...
  // Hamburger menu + breadcrumb móvil
  TB_injectHamburger();
  TB_setupMobileBreadcrumb();
  TB_injectFooterPaymentIcons();
  ...
  // Sincronizar badge cuando cambia el carrito en otra pestana (solo guest)
  window.addEventListener("storage", (e) => {
    if (e.key === "techbuy_cart" && !TB_api.isLoggedIn()) {
...
function TB_injectHamburger() {
  if (document.getElementById("tbHamburger")) return;

  // Inject hamburger button into header
  const headerInner = document.querySelector(".tb-header-inner");
  if (!headerInner) return;

  const hamBtn = document.createElement("button");
  hamBtn.className = "tb-hamburger";
  hamBtn.id = "tbHamburger";
  hamBtn.setAttribute("aria-label", "Menú");
  hamBtn.innerHTML = "<span></span><span></span><span></span>";
  headerInner.insertBefore(hamBtn, headerInner.firstChild);

  // Inject menu panel + overlay
  const panel = document.createElement("div");
  panel.className = "tb-menu-panel";
  panel.id = "tbMenuPanel";
  panel.innerHTML = `
    <div class="tb-menu-header">
      <img src="../img/logo.webp" alt="TechBuy" onerror="this.style.display='none'">
      <span>TechBuy</span>
      <button class="tb-menu-close" id="tbMenuClose"><i class="fa-solid fa-xmark"></i></button>
    </div>
    <nav class="tb-menu-nav">
      <a class="tb-menu-link" href="1.html"><i class="fa-solid fa-house"></i> Inicio</a>
      <a class="tb-menu-link" href="categorias.html"><i class="fa-solid fa-boxes-stacked"></i> Categorías</a>
      <a class="tb-menu-link" href="ofertas.html"><i class="fa-solid fa-tags"></i> Ofertas</a>
      <a class="tb-menu-link" href="contacto.html"><i class="fa-solid fa-headset"></i> Contáctanos</a>
    </nav>`;
  document.body.appendChild(panel);

  const overlay = document.createElement("div");
  overlay.className = "tb-menu-overlay";
  overlay.id = "tbMenuOverlay";
  document.body.appendChild(overlay);

  // Mark active link
  const currentPath = window.location.pathname.split("/").pop() || "1.html";
  panel.querySelectorAll(".tb-menu-link").forEach(a => {
    if (a.getAttribute("href") === currentPath) a.classList.add("active");
  });

  // Toggle
  const closeBtn = document.getElementById("tbMenuClose");
...
/* ─── MOBILE BREADCRUMB ──────────────────────────────────── */
function TB_setupMobileBreadcrumb() {
  if (window.innerWidth > 600) return;
  const bcInner = document.querySelector(".tb-bc-inner");
  if (!bcInner) return;
  // Find the last link before current (penultimate link = back target)
  const links = bcInner.querySelectorAll(".tb-bc-link");
  let backHref = "categorias.html";
  let backText = "Volver al catálogo";
  if (links.length >= 2) {
    backHref = links[links.length - 1].getAttribute("href");
    backText = links[links.length - 1].textContent.trim();
  } else if (links.length === 1) {
    backHref = links[0].getAttribute("href");
    backText = links[0].textContent.trim();
  }
  // Insert back link if not already present
  if (!bcInner.querySelector(".tb-bc-back")) {
    const back = document.createElement("a");
    back.className = "tb-bc-back";
    back.href = backHref;
    back.innerHTML = `<i class="fa-solid fa-arrow-left"></i> ${backText}`;
    bcInner.prepend(back);
  }
}
...
async function TB_getSuggestions(query) {
  if (!query.trim()) return [];
  try {
    const data = await TB_api.get('/search/suggest?q=' + encodeURIComponent(query.trim()) + '&limit=' + MAX_SUGGESTIONS);
    return Array.isArray(data) ? data : [];
  } catch (e) {
    console.warn('Error en autocompletado:', e.message);
    return [];
  }
}
...
function TB_renderSuggestions(items, query) {
  ...
  const listHtml = items.map((p, i) => {
    const imgUrl = p.imagen || (p.imagenes && p.imagenes[0]) || null;
    return `
    <div class="tb-sug-item" data-idx="${i}" role="option">
      ${imgUrl
        ? `<img class="tb-sug-img" src="${imgUrl}" alt="" onerror="this.style.display='none'">`
  ...
  const seeAllHtml = `<a class="tb-sug-see-all" href="categorias.html?q=${encodeURIComponent(query)}">
    Ver todos los resultados <i class="fa-solid fa-arrow-right"></i>
  </a>`;
  container.innerHTML = listHtml + seeAllHtml;
...
function TB_handleSearchInput(e) {
  ...
  acDebounceTimer = setTimeout(async () => {
    acSuggestions = await TB_getSuggestions(query);
    acActiveIdx = -1;
  ...
  <div class="acct-modal" id="authModal">
    ...
    <div id="formLogin" class="auth-panel">
      <div class="auth-brand">
        <img src="../img/logo.webp" alt="TechBuy" class="auth-logo-img" onerror="this.style.display='none'">
...
/* ══════════════════════════════════════════════════════════════
   SHARED PAGINATION — componente reutilizable
   Dependencias: ninguna
   CSS: shared.css (.tb-pagination, .tb-page-btn, .tb-page-dots)
══════════════════════════════════════════════════════════════ */
const TB_PAGE_SIZE = 12;

function TB_renderPagination({ container, currentPage, totalPages, onPageChange, containerId }) {
  if (typeof container === 'string') container = document.getElementById(container);
  if (!container) return;

  const id = containerId || 'tbPagination';
  let pagEl = document.getElementById(id);
  if (!pagEl) {
    pagEl = document.createElement('div');
    pagEl.id = id;
    pagEl.className = 'tb-pagination';
  ...
  let html = `<button class="tb-page-btn tb-page-prev" data-page="${currentPage - 1}"${prevDisabled ? ' disabled' : ''}>&#8249; Anterior</button>`;

  const startPage = Math.max(1, currentPage - 2);
  const endPage = Math.min(totalPages, currentPage + 2);

  if (startPage > 1) {
    html += `<button class="tb-page-btn" data-page="1">1</button>`;
    if (startPage > 2) html += `<span class="tb-page-dots">…</span>`;
  }
  for (let i = startPage; i <= endPage; i++) {
    html += `<button class="tb-page-btn${i === currentPage ? ' active' : ''}" data-page="${i}">${i}</button>`;
  }
  if (endPage < totalPages) {
    if (endPage < totalPages - 1) html += `<span class="tb-page-dots">…</span>`;
    html += `<button class="tb-page-btn" data-page="${totalPages}">${totalPages}</button>`;
  }
  html += `<button class="tb-page-btn tb-page-next" data-page="${currentPage + 1}"${currentPage >= totalPages ? ' disabled' : ''}>Siguiente &#8250;</button>`;

  pagEl.innerHTML = html;

  pagEl.removeEventListener('click', pagEl._tbHandler);
  pagEl._tbHandler = function(e) {
    const btn = e.target.closest('.tb-page-btn');
    if (!btn || btn.disabled) return;
    const page = parseInt(btn.dataset.page, 10);
    if (!isNaN(page)) onPageChange(page);
  };
  pagEl.addEventListener('click', pagEl._tbHandler);
}

/* ─── FOOTER PAYMENT ICONS (móvil) ────────────────────── */
window.TB_injectFooterPaymentIcons = function() {
  if (window.innerWidth > 480) return;
  const sec = document.querySelector('.foot-grid > :nth-child(4)');
  if (!sec || sec.querySelector('.foot-payment')) return;

  const info = document.createElement('div');
  info.className = 'foot-contact-info';
  while (sec.firstChild) info.appendChild(sec.firstChild);
  sec.appendChild(info);

  const pay = document.createElement('div');
  pay.className = 'foot-payment';
  pay.innerHTML = '<div class="foot-title">M\u00e9todos de Pago</div><div class="foot-payment-icons">' +
